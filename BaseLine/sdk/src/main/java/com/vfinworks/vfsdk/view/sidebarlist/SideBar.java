...
public class SideBar extends View {
    ...
    protected void onDraw(Canvas canvas) {  
        ...
        for (int i = 0; i < A_Z.length; i++) {  
            paint.setColor(Color.rgb(66, 133, 244));  //设置字体颜色
            paint.setTypeface(Typeface.DEFAULT_BOLD);  //设置字体
            paint.setAntiAlias(true);  //设置抗锯齿
            paint.setTextSize(Utils.getInstance().dip2px(mContext,12));  //设置字母字体大小
            // 选中的状态  
            if (i == choose) {  
                paint.setColor(Color.parseColor("#3399ff"));  //选中的字母改变颜色
                paint.setFakeBoldText(true);  //设置字体为粗体
            }
            getFontY(paint.getFontMetrics());
            // x坐标等于中间-字符串宽度的一半.
            float xPos = width / 2 - paint.measureText(A_Z[i]) / 2;
            float yPos = singleHeight * i+fontY;
    ...
    private void getFontY(Paint.FontMetrics fm) {
        fm = paint.getFontMetrics();
        fontY=(int) Math.ceil(fm.descent - fm.top) ;
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {  
        ...
        default:  
            setBackgroundResource(R.drawable.vf_sidebar_background);
