...
//    private int mResId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_entertransition); // 布局代码和5.0共用一个
        mTargetImageView = (ImageView) findViewById(R.id.imageDetail);
        mContainer = (FrameLayout) findViewById(R.id.activityContanierDetail);
        initSourceImageView();
...
//        mResId = bundle.getInt(IMAGE_RES_ID);
        mBitmap =ImageBitmap.getInstance().getBitmap();

        // 设置为和第一个界面一样的图片
        mSourceImageView.setImageBitmap(mBitmap);
        mTargetImageView.setImageBitmap(mBitmap);
