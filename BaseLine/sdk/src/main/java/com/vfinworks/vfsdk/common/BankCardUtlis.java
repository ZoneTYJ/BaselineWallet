...
/**
 * Created by tangyijian on 2016/8/19.
 */

public class BankCardUtlis {
    ...
    private final static int[] bankBin = {102033, 103000, 185720, 303781,
            ...
            998800,};
    // "发卡行.卡种名称",
    private static final String[] bankDetailName = {"广东发展银行.广发理财通", "农业银行.金穗借记卡",
    ...
    public static String getNameOfBank(String charBin) {
        charBin = charBin.replace(" ", "");
        int bin = 0, index = 0;
        charBin = charBin.substring(0, 6);
        bin = Integer.valueOf(charBin);
        index = binarySearch(bankBin, bin);
        if (index == -1 || index > bankDetailName.length) {
            return "没有记录的卡号";
        }
        return bankDetailName[index];
    ...
    private static final String[] bankNames = {"人民银行", "工商银行", "建设银行", "汇丰银行", "中国银行",
            "农业银行", "交通银行", "招商银行", "民生银行", "浦东发展银行", "中信", "中国光大银行", "华夏银行", "广东发展银行",
            "深圳发展银行", "兴业银行",
            "国家开发银行", "进出口银行", "农业发展银行", "北京银行"};
    private static final String[] abridges = {"PBC", "ICBC", "CCB", "HSBC", "BC",
            "ABC", "BC", "CMB", "CMB", "SPDB", "", "CEB", "HB", "GDB", "SDB", "CIB",
            "CDB", "EIBC", "ADBC", "BOB"};

    public static String getBankCodeOfName(String bankName) {
        String strs[] = bankName.split("\\.");
        if (strs.length == 0) {
            return null;
        }
        int i = 0;
        for (i = 0; i < bankNames.length; i++) {
            if (strs[0].contains(bankNames[i])) {
                break;
            }
        }
        String bankcode;
        if (i < bankNames.length) {
            bankcode = abridges[i];
        } else {
            bankcode = "";
        }
    ...
    public static String getBankCodeOfBin(String bin){
           return getBankCodeOfName(getNameOfBank(bin));
    }
